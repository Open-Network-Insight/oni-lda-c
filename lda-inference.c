...
double lda_inference(document* doc, lda_model* model, double* var_gamma, double** var_phi)
{
    double converged = 1;
    ...
    for (k = 0; k < model->num_topics; k++)
    {
        ...
        for (n = 0; n < doc->length; n++)
            var_phi[n][k] = 1.0/model->num_topics;
    }
    var_iter = 0;
	...
	for (n = 0; n < doc->length; n++)
	{
            ...
            for (k = 0; k < model->num_topics; k++)
            {
                ...
                if (k > 0)
                    ...
                    phisum = var_phi[n][k]; // note, phi is in log space
            }

            for (k = 0; k < model->num_topics; k++)
            {
                var_phi[n][k] = exp(var_phi[n][k] - phisum);
                var_gamma[k] =
                    var_gamma[k] + doc->counts[n]*(var_phi[n][k] - oldphi[k]);
        ...
        likelihood = compute_likelihood(doc, model, var_phi, var_gamma);
